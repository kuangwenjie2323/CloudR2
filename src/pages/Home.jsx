    ...
    <div className="p-4 space-y-4">
      ...
      {folders?.length > 0 && (
        <div className="flex flex-wrap gap-2">
          ...
          {prefix && (
            <button onClick={() => setPrefix(prefix.split("/").slice(0,-2).join("/") + (prefix.includes("/")?"/":""))}
                    className="px-3 py-2 rounded bg-zinc-100 hover:bg-zinc-200">⬆ 上级</button>
          )}
        </div>
      )}
      ...
      {view === "grid" ? (
        <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-4">
          {items.map((o) => (
            <div key={o.key} className="rounded-2xl p-3 bg-white shadow-sm border border-zinc-100">
        ...
        <div className="rounded-xl overflow-hidden border border-zinc-200">
          <table className="w-full text-sm">
            <thead className="bg-zinc-50">
              <tr><th className="text-left p-2">文件名</th><th className="text-left p-2">大小</th><th className="text-left p-2">时间</th></tr>
            </thead>
            <tbody>
              {items.map((o) => (
